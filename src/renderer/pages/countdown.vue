<template>
  ...
  >
    <div
      class="text-center text-time font-mono"
      :style="{
        color: timerText
      }"
    >
    ...
    <div class="relative pt-1 px-5">
      <div
        ...
        :class="{
          ...
          'bg-emerald-200': !isCountingUp && !isReset,
          'bg-gray-200': isReset
        }"
      >
        <div
          ...
          :class="{
            ...
            'bg-emerald-500': !isCountingUp && !isReset
          }"
        />
      </div>
    </div>
    <div class="text-center text-clock font-mono">
...
export default {
  ...
  computed: {
    time () {
      return dayjs.duration(Math.abs(this.update.current), 'seconds').format('HH:mm:ss')
    },
    progressBarPercent () {
