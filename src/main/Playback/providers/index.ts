import {GRANDSHOW_PROVIDER_ID, MILLUMIN_PROVIDER_ID, OSCPOINT_PROVIDER_ID, QLAB_PROVIDER_ID, VMIX_PROVIDER_ID} from "@common/playback.ts";
...
import {OscPointProvider} from "./oscpoint/OscPointProvider.ts";
import {GrandShowProvider} from "./grandshow/GrandShowProvider.ts";
...
export const playbackProviderFactories: {[providerId: string]: PlaybackProviderFactory} = {
  ...
  [OSCPOINT_PROVIDER_ID]: (context) => new OscPointProvider(context),
  [GRANDSHOW_PROVIDER_ID]: (context) => new GrandShowProvider(context),
}
